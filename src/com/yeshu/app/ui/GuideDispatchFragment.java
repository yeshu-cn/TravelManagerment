...
public class GuideDispatchFragment extends BaseFragment {
	...
	private final int FIRST_PAGE = 1;//团队列表的第一页
	private int CURRENT_PAGE = 1;	//已经加载到的页面序号
			...
			public void onSuccess(JSONArray response) {
				try {
					...
					if(tourGroupDatas.isEmpty()){
						//为空说明是最后的数据了,CURRENT_PAGE不在增加,所以-1；
						CURRENT_PAGE--;
						Toast.makeText(getActivity(), "已是最后的数据！", Toast.LENGTH_SHORT).show();
					}else{
						mAdapter.addData(tourGroupDatas);
					}
...
//					Toast.makeText(getActivity(), "数据加载完成.", Toast.LENGTH_SHORT).show();
	...
	private void showLoadingUI() {
		...
		loadingViewLable.setText("拼命获取数据中....");
